...
**Resolved on 2026-09-24:** added the page's explicit "Move to Done" action for a selected `needs_review` card, routed through `moveCard(cardId, "done", "user")`, with English/Simplified Chinese labels and an integration test. The E7 manual workflow can now be attempted; its host-side acceptance remains pending below.
