...
`openchamber-loop-kanban` 是一个 OpenChamber 扩展，不是独立桌面应用。它在右侧 rail 提供轻量看板面板，并由用户从 **Extension pages** 打开完整看板页。
...
| `docs/superpowers/specs/2026-09-22-openchamber-loop-kanban-extension-design.md` | 面板、完整页面、数据模型、权限和已知 SDK 能力边界。 |
| `docs/traceability.md` | 每项需求对应的实现阶段和验收证据。 |
| `docs/superpowers/plans/2026-09-22-openchamber-loop-kanban-extension-plan.md` | 严格分阶段实施清单。 |

## 固定约束

- 包名、面板 ID 和项目逻辑名均为 `openchamber-loop-kanban`。
