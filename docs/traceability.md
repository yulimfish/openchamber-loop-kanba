...
| EXT-006 | 扩展只使用官方 SDK v1 和最小 `sessions`、`prompt` 权限。 | ADR-001：决策 | E0, E1, E4, E7 | E0 已通过 `bun test tests/manifest.test.ts` 与 `bunx tsc --noEmit`；E4 workflow 仅经 Host adapter 调用 `startSession`/`openSession`/`listSessions`/`onSessions`；source scan 待 E7。 |
...
| EXT-008 | 自动 Coordinator/Goal/Review loop 仅在公开 Host API 发布后实现。 | ADR-001：上游前置条件 | E5, E6 | E5 已通过 `automationState()` 四契约 fail-closed 测试；E6 为 SDK 发布后的发布闸门。 |
