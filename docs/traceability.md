...
| EXT-006 | 扩展只使用官方 SDK v1 和最小 `sessions`、`prompt` 权限。 | ADR-001：决策 | E0, E1, E4, E7 | manifest capability snapshot 与 source scan。 |
...
| EXT-010 | 扩展不声明 local service、filesystem、files、model、network 或 conversation。 | ADR-001：数据与权限 | E0, E7 | manifest parser、capability snapshot 与 install dialog test。 |
